...
# azure.cgi -
#
# Get/set EC-Azure configuration info for UI
#
...
################################
# getCfgList - Return the list of configurations from Azure
#
# Arguments:
#   -
#
...
sub getCfgList {
    ...
    my $gcfg = new ElectricCommander::PropDB($::ec,"/projects/@PLUGIN_NAME@/azure_cfgs");
    ...
    foreach my $cfg (keys  %cfgs) {
        my $description = $gcfg->getCol("$cfg/description");
        $xml .= "  <cfg>\n";
        $xml .= "     <name>$cfg</name>\n";
        $xml .= "     <description>" . xmlQuote($description) . "</description>\n";
        $xml .= "  </cfg>\n";
    }
    $xml .= "</cfgs>\n";

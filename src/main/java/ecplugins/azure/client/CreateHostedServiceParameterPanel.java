...
public class CreateHostedServiceParameterPanel extends ComponentBase implements
        ParameterPanel, ParameterPanelProvider, HasInitCallback {
    // ~ Static fields/initializers
    // ---------------------------------------------
    private static UiBinder<Widget, CreateHostedServiceParameterPanel> s_binder = GWT
            .create(Binder.class);
    ...
    // --------------------------------------------------------
    @UiField
    FormBuilder create_hosted_service_ParameterForm;
    ...
    public Widget doInit() {
        ...
        create_hosted_service_ParameterForm
                .addRow(true,
                        "Configuration:",
    ...
    public boolean validate() {
        boolean validationStatus = create_hosted_service_ParameterForm
                .validate();

        // CONNECTION_CONFIG required.
        if (StringUtil.isEmpty(create_hosted_service_ParameterForm.getValue(CONNECTION_CONFIG).trim())) {
            create_hosted_service_ParameterForm
        ...
        // LABEL required.
        if (StringUtil.isEmpty(create_hosted_service_ParameterForm.getValue(LABEL).trim())) {
            create_hosted_service_ParameterForm
        ...
        if (StringUtil.isEmpty(create_hosted_service_ParameterForm.getValue(SERVICE_NAME).trim())) {
            create_hosted_service_ParameterForm
        ...
        if (StringUtil.isEmpty(create_hosted_service_ParameterForm.getValue(
                LOCATION).trim())
                & StringUtil.isEmpty(create_hosted_service_ParameterForm
    ...
    protected void updateRowVisibility() {
        ...
        create_hosted_service_ParameterForm.setRowVisible(AFFINITY_GROUP,
                StringUtil.isEmpty(create_hosted_service_ParameterForm
                        .getValue(LOCATION).trim()));
        create_hosted_service_ParameterForm.setRowVisible(LOCATION, StringUtil
                .isEmpty(create_hosted_service_ParameterForm.getValue(
                        AFFINITY_GROUP).trim()));
        create_hosted_service_ParameterForm.setRowVisible(VALUE, !StringUtil
                .isEmpty(create_hosted_service_ParameterForm.getValue(NAME)
                        .trim()));
    ...
    public Map<String, String> getValues() {
        ...
        Map<String, String> create_hosted_service_FormValues = create_hosted_service_ParameterForm
                .getValues();
    ...
    public void setActualParameters(Collection<ActualParameter> actualParameters) {
        ...
        for (String key : new String[] { CONNECTION_CONFIG, SERVICE_NAME,
                LABEL, DESCRIPTION, LOCATION, AFFINITY_GROUP, NAME, VALUE,
                RESULTS_LOCATION_OUTPSP, TAG_OUTPP, JOB_STEP_TIMEOUT }) {
    ...
    interface Binder extends
            UiBinder<Widget, CreateHostedServiceParameterPanel> {
    }
}
